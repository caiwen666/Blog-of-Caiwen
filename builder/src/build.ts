...
let articleList: ArticleList = []; //data.json
let articleMap: ArticleMap = {}; //index.json
let category: Category | null = null; //tree.json
let recommend: Recommend | null = null;
let home: string[] = []; //home.json
let discard: string[] = []; //discard.json
let draft: string[] = []; //draft.json
let cache: { [key: string]: BuildCache } = {}; // 指定 id 的文章内容的 md5，用于判断是否要重新编译，cache.json

//构建整个博客的数据
//文章的所有数据应存储在工作目录/main中
...
export async function build(
	...
	category = (await buildTree(Path.join(path, "main"), [], path)) as Category;
	//文章排序
